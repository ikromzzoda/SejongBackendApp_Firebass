...
В поле `username` можно передать **логин или email** — строка с `@` сначала ищется по почте (без учёта регистра), затем по логину.
...
Для **Admin** в ответ дополнительно добавляется `unread_contact_messages` — число непрочитанных обращений «связаться с админом» (`POST /api/announcements/contact/`). Если оно больше нуля, `message` содержит уведомление: `"Вход выполнен. У вас N непрочитанных обращений от пользователей."` Список обращений: `GET /api/announcements/admin/contact-messages/`.

**Ошибки:**

| Код | Ответ |
|-----|-------|
| 400 | `{"error": "Введите username или email, пароль и device_token"}` |
| 401 | `{"error": "Неверный username/email или пароль"}` |
...
Отправляет 6-значный код на email. Код действует **15 минут**, повторная отправка — не чаще **раза в 10 минут**. Если аккаунта с такой почтой нет — возвращается 404.
...
|-----|-------|
| 400 | `{"error": "Поле \"email\" обязательно"}` |
| 404 | `{"error": "Аккаунт с такой почтой не существует. Введите актуальную почту."}` |
