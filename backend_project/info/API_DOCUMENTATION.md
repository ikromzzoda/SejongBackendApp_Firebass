...
Все list-эндпоинты поддерживают пагинацию через query-параметры:

| Параметр | По умолчанию | Максимум | Описание |
|----------|:---:|:---:|----------|
| `limit` | `100` | `500` | Количество записей на странице |
| `offset` | `0` | — | Смещение от начала отсортированного списка |

Ответ всегда содержит поле `total` — общее число записей до применения пагинации.

**Пример:** `GET /api/info/schedules/all/?limit=20&offset=40`
...
| GET | `/admin/schedules/` | Admin | Список расписаний (с пагинацией) |
...
| GET | `/schedules/all/` | Авторизованный | Все расписания (с пагинацией) |
...
| GET | `/admin/notifications/` | Admin | Список уведомлений (с пагинацией) |
...
Список расписаний. Можно фильтровать по группе или преподавателю (только один параметр за раз). Поддерживает пагинацию.

**Query параметры:**

| Параметр | Обязательное | Пример | Описание |
|----------|:---:|--------|----------|
| `group_name` | ❌ | `?group_name=CS-101` | Фильтр по группе |
| `teacher_name` | ❌ | `?teacher_name=Иванов Иван` | Фильтр по преподавателю |
| `limit` | ❌ | `?limit=50` | Записей на странице (по умолч. 100, макс. 500) |
| `offset` | ❌ | `?offset=100` | Смещение |
...
Все расписания для авторизованного пользователя. Отсортированы по дню недели и времени начала. Поддерживает пагинацию.
...
Список всех уведомлений, отсортированных от новых к старым. Поддерживает пагинацию.
...
> При передаче новых `images` — сначала загружаются новые файлы, и только после успешной загрузки старые изображения удаляются с Google Drive. Если загрузка не удалась, старые изображения остаются без изменений.
...
| 502 | Ошибка загрузки изображения на Google Drive |
...
Уведомления текущего пользователя. Возвращает только те, где статус пользователя входит в `target_statuses`. Отсортированы от новых к старым. Поддерживает пагинацию.

**Query параметры:**

| Параметр | Обязательное | Пример | Описание |
|----------|:---:|--------|----------|
| `limit` | ❌ | `?limit=20` | Записей на странице (по умолч. 100, макс. 500) |
| `offset` | ❌ | `?offset=0` | Смещение |
...
### Список расписаний с пагинацией
```
GET http://127.0.0.1:8000/api/info/admin/schedules/?limit=20&offset=0
Authorization: Bearer <admin_token>
```
