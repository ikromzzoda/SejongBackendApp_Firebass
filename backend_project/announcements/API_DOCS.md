...
Список объявлений с поддержкой пагинации.

**Query параметры:**

| Параметр | Тип | По умолчанию | Описание |
|----------|-----|:---:|----------|
| `limit` | integer | `20` | Количество объявлений на страницу (максимум 100) |

Чтобы получить следующую страницу — передайте `limit` снова и используйте `id` последнего элемента текущей страницы как ориентир.
...
> `has_more: true` означает, что в базе есть ещё объявления сверх возвращённых.
...
| 502 | Ошибка сохранения в Firestore | `{"error": "Ошибка сохранения объявления: ..."}` |

> При ошибке сохранения в Firestore все уже загруженные на Drive изображения автоматически удаляются.
...
### Список объявлений (первая страница)
```
GET http://127.0.0.1:8000/api/announcements/?limit=20
Authorization: Bearer <token>
```

### Список объявлений (следующая страница)
```
GET http://127.0.0.1:8000/api/announcements/?limit=20
Authorization: Bearer <token>
```
